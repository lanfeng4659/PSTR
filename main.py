...
def main():
  ...
  if not run_on_remote:
    print(args)
    cmd_pretrain = "CUDA_VISIBLE_DEVICES=1,3 python -m torch.distributed.launch --nproc_per_node={0} --master_port 20000 tools/train_net.py \
           --config-file {1} \
    ...
    cmd_finetune = "CUDA_VISIBLE_DEVICES=2,3 python -m torch.distributed.launch --nproc_per_node={0} --master_port 20000 tools/train_net.py \
          --config-file {1} \
    ...
    # cache_dataset_path = '/home/ma-user/modelarts/inputs/data_url_0'
    # mox.file.copy_parallel(args.datasets_path, cache_dataset_path)
    # datasets = ['SynthText_90KDict','MLT2019','TotalText']
    # # datasets = ['totaltext']
  ...
  # os.system("ln -s {} datasets".format(args.data_url))
  # print(args.data_url)
  # print(os.listdir(args.data_url))
  
  # print(cmd_pretrain)
  # os.system(cmd_pretrain)

  print(cmd_model_process)
  os.system(cmd_model_process)
  ...
  # iiit_test = "CUDA_VISIBLE_DEVICES=1 python {0} --config-file {1} \
  #  --OUTPUT_DIR {2}/finetune \
  #   --dataset_id {3}".format(run_file_path, args.finetune_config, args.output_dir, 0) # iiit
  # coco_test = "CUDA_VISIBLE_DEVICES=1 python {0} --config-file {1} \
  #  --OUTPUT_DIR {2}/finetune \
  #   --dataset_id {3}".format(run_file_path, args.finetune_config, args.output_dir, 1) # iiit
  # svt_test = "CUDA_VISIBLE_DEVICES=1 python {0} --config-file {1} \
  #  --OUTPUT_DIR {2}/finetune \
  #   --dataset_id {3}".format(run_file_path, args.finetune_config, args.output_dir, 2) # iiit
  # tt_test = "CUDA_VISIBLE_DEVICES=1 python {0} --config-file {1} \
  #  --OUTPUT_DIR {2}/finetune \
