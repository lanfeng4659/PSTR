...
INF = 10000000000
def get_max(reference, key):
    value, index = reference.max(dim=1)
    index = torch.tensor(list(range(reference.size(0)))).type_as(index)*reference.size(1) + index
    kv = key.view(-1)[index]
    return value, kv
def get_min(reference, key):
    value, index = reference.min(dim=1)
    index = torch.tensor(list(range(reference.size(0)))).type_as(index)*reference.size(1) + index
    kv = key.view(-1)[index]
    return value, kv
...
class AlignHead(nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.use_mil = cfg.MODEL.ALIGN.USE_MIL
    ...
    def get_image_embedding(self,proposals_per_im,x,img_lens=None, is_partial=False, images=None):
        ...
        if proposals_per_im.bbox.size(0) == 0:
            ...
            imgs_embedding = torch.zeros([0,15,128]).type_as(x[0])
        else:
        ...
        return imgs_embedding_nor, att, select_boxes,imgs_embedding
    ...
    def compute_loss(self, embedding1, embedding2, targets=None, words1=None,words2=None, using_sim_max=False):
        k = 1
        ...
        if not using_sim_max:
            loss = self.sim_loss_func(iou, similarity)
            loss = loss.max(dim=1)[0].mean()
        else:
            iv_max, sv_max = get_max(iou, similarity)
            sv_max = torch.ones_like(sv_max)
            # iv_min, sv_min = get_min(iou, similarity)
            # iv = torch.cat([iv_max, iv_min])
            # sv = torch.cat([sv_max, sv_min])
            loss = self.sim_loss_func(iv_max, sv_max).mean()
        return loss
    def compute_similarity(self,embedding1, embedding2,k=1):
        ...
        return inter
    def compute_element_similarity(self,embedding1, embedding2,k=1):
        embedding1_nor = nn.functional.normalize((embedding1*k).tanh().view(embedding1.size(0),-1))
        embedding2_nor = nn.functional.normalize((embedding2*k).tanh().view(embedding2.size(0),-1))
        inter = (embedding1_nor * embedding2_nor).sum(dim=-1)
        return inter
    ...
    def training_loss(self,rois, imgs_texts, word_texts, imgs_embedding, words_embedding, re, we, use_domain=True, phase=''):
        def wrap_name(name):
        ...
        wi_loss = loss_fn(words_embedding.detach(), imgs_embedding, wi_sim)
        ww_loss = loss_fn(words_embedding, words_embedding, ww_sim)
        ii_loss = loss_fn(imgs_embedding, imgs_embedding, ii_sim)
        ...
        if use_domain:
            if not self.use_mil:
                ...
                loss[wrap_name("loss_da")] = self.compute_domain_align_loss(imgs_embedding, words_embedding.detach())
                loss[wrap_name("loss_dab")] = self.decoder(re, we.detach())/4
        return loss
    def training_loss_mil(self,rois, imgs_texts, word_texts, imgs_embedding, words_embedding, ids_of_parttofull, texts_of_partoffull,feats_of_partoffull=None, phase=''):
        ...
        def sum_to_zero_loss(feats):
            ...
            return loss
        def query_bag_loss():
            # print(ids_of_parttofull, ids_of_parttofull.max())
            max_bag_id = ids_of_parttofull.max()
            loss = []
            for bag_id in range(max_bag_id):
                idx = torch.nonzero(ids_of_parttofull==bag_id)
                if idx.numel()==0: continue
                query_feats_in_the_bag = words_embedding[idx]
                img_feats_in_the_bag = imgs_embedding[idx]

                idx_list = idx.view(-1).data.cpu().numpy().astype(np.int32)
                # print(idx_list)
                query_texts = [word_texts[v] for v in idx_list]
                image_texts = [imgs_texts[v] for v in idx_list]
                # print(query_texts, image_texts)
                wi_sim = torch.tensor(self.text_generator.calculate_similarity_matric(query_texts, image_texts)).to(rois.device)
                wi_loss = self.compute_loss(query_feats_in_the_bag.detach(), img_feats_in_the_bag, wi_sim, using_sim_max=True)
                loss.append(wi_loss)
            if len(loss)==0:
                return sum_to_zero_loss([imgs_embedding, words_embedding])
            return sum(loss)/len(loss)
        def query_bag_margin_ranking_loss():
            margin = 1- torch.diag(torch.tensor(self.text_generator.calculate_similarity_matric(word_texts, texts_of_partoffull)).to(rois.device)).view(-1)
            query_instance = self.compute_element_similarity(words_embedding.detach(), imgs_embedding).view(-1)
            query_full = self.compute_element_similarity(words_embedding.detach(), feats_of_partoffull).view(-1)
            margin = torch.zeros_like(margin) + 0.2
            # loss(x1,x2,y)=max(0,−y∗(x1−x2)+margin) y=1,:  x1 > x2
            loss = margin - (query_instance - query_full)  
            loss = loss[torch.nonzero(loss > 0)].view(-1)
            if loss.numel()==0:
                return sum_to_zero_loss([imgs_embedding, words_embedding])
            # print(loss, sum(loss))
            return sum(loss)/len(loss)
        if imgs_embedding.size(0)<2:
            zero_loss = sum_to_zero_loss([rois, imgs_embedding, words_embedding])
            loss = {wrap_name("loss_wi_mil"):zero_loss,wrap_name("loss_ww_mil"):zero_loss,wrap_name("loss_mg_mil"):zero_loss}
            return loss
        ...
        loss_fn = self.compute_loss
        wi_loss = loss_fn(words_embedding.detach(), imgs_embedding, wi_sim, using_sim_max=True)
        mg_loss = query_bag_margin_ranking_loss() #query_bag_loss()
        ww_loss = loss_fn(words_embedding, words_embedding, ww_sim, using_sim_max=False)
        # ii_loss = loss_fn(imgs_embedding, imgs_embedding, ii_sim, using_mask=True)
        # print(wi_loss,ww_loss,mg_loss)
        loss = {"loss_wi_mil":wi_loss*10,"loss_ww_mil":ww_loss*10,"loss_mg_mil":mg_loss*5}
        return loss
    ...
    def split_full_partial(self, proposals, device):
        ...
        for idx, proposals_per_im in enumerate(proposals):
            ...
            offset += all_num
        return idxs
    def construct_bags(self, proposals, device):
        # given each path a full instance id
        ids = []
        offset = 0
        for idx, proposals_per_im in enumerate(proposals):
            bag_ids = proposals_per_im.get_field("bag_ids")
            bag_num = bag_ids[0]
            instance_offset_in_img = bag_num
            for idx, instance_num_in_img in enumerate(bag_ids[1:]):
                ids = ids + [idx+offset]*(instance_num_in_img-instance_offset_in_img)
                instance_offset_in_img = instance_num_in_img
            offset += bag_num
    ...
    def forward(self, x, samples,images=None,is_words=None):
        ...
        if self.training:
            ...
            assert len(rois)==len(texts), print(len(rois),len(texts))
            ...
            if self.use_mil:
                ...
                full_img_texts = [imgs_texts[v] for v in range(len(imgs_texts)) if v in full_idxs_list]
                full_word_texts= [word_texts[v] for v in range(len(word_texts)) if v in full_idxs_list]
                loss = self.training_loss(rois[full_idxs], \
                                          full_img_texts,\
                                          full_word_texts,\
                                          imgs_embedding[full_idxs],\
                                          words_embedding[full_idxs], re[full_idxs], we[full_idxs])
                ...
                ids_of_parttofull = self.construct_bags(proposals, device=re.device)
                ids_of_parttofull_list = ids_of_parttofull.data.cpu().numpy()
                texts_of_partoffull = [full_img_texts[v] for v in ids_of_parttofull_list]
                # for v in zip(part_img_texts, texts_of_partoffull):
                #     print(v)
                if len(ids_of_parttofull_list)==0:
                    loss_mil = words_embedding.view(-1).sum()*0
                else:
                    # if len(rois[part_idxs])!=len(part_img_texts):
                    # print(len(rois),len(part_img_texts),part_idxs.max())
                    loss_mil = self.training_loss_mil(rois[part_idxs], \
                                                part_img_texts,\
                                                part_word_texts,\
                                                imgs_embedding[part_idxs],\
                                                words_embedding[part_idxs],\
                                                ids_of_parttofull,\
                                                texts_of_partoffull,\
                                                feats_of_partoffull=imgs_embedding[full_idxs][ids_of_parttofull],\
                                                phase='mil_')
                # print(loss, loss_mil)
                # loss.update(loss_mil)
                for k, v in loss_mil.items():
                    assert k not in loss.keys()
                    loss[k] = v
                # self.mil_loss(imgs_embedding, words_embedding, proposals)
            else:
                loss = self.training_loss(rois, imgs_texts, word_texts, imgs_embedding, words_embedding, re, we, use_domain=self.use_domain_align_loss)
            ...
            for proposals_per_im in proposals:
                ...
                if is_words:
                    ...
                    if len(texts) == 0:
                        ...
                        # proposals_per_im.add_field("words_embedding_nor", words_embedding_nor)
                        proposals_per_im.add_field("words_embedding", words_embedding)
                if self.use_len_embed:
                    img_lens = proposals_per_im.get_field("lens")
                    word_lens = torch.tensor([min(len(v), 19) for v in texts]).long().to(x[0].device)
                else:
                    img_lens, word_lens = None, None
                imgs_embedding_nor, att, select_boxes,imgs_embedding = self.get_image_embedding(proposals_per_im,x,img_lens=img_lens, is_partial=True)
                # imgs_embedding_nor, att, select_boxes = self.get_image_embedding(proposals_per_im,x, is_partial=True, images=images)
                # proposals_per_im.add_field("imgs_embedding_nor", imgs_embedding_nor)
                proposals_per_im.add_field("imgs_embedding", imgs_embedding)
                # proposals_per_im.add_field("imgs_feats", x)
                # if self.use_scalenet:
                #     proposals_per_im.add_field("attention", att)
