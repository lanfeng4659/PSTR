...
def resnet18(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = {k:v for k,v in torch.load(cache_url(model_urls['resnet18'][1])).items() if 'fc' not in k}
...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        state_dict = {k:v for k,v in torch.load(cache_url(model_urls['resnet50'][1])).items() if 'fc' not in k}
        # state_dict = {k:v for k,v in torch.load("/home/ymk-wh/workspace/researches/wanghao/retrieval-models/resnet50-19c8e357.pth").items() if 'fc' not in k}
        model.load_state_dict(state_dict)
    return model
