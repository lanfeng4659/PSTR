...
def build_resnet_50_bn_backbone(cfg):
    # model = resnet_bn.resnet50(pretrained=True)
    model = resnet_bn.resnet50(pretrained=False)
    # model = nn.Sequential(OrderedDict([("body", body)]))
    model.out_channels = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS
    return model

@registry.BACKBONES.register("R-18")
def build_resnet_18_backbone(cfg):
    print("R1888888888888")
    model = resnet_bn.resnet18(pretrained=True)
    # model = nn.Sequential(OrderedDict([("body", body)]))
    model.out_channels = cfg.MODEL.RESNETS.BACKBONE_OUT_CHANNELS
    return model
