...
class LSVT(object):
    ...
    def generate_information(self, path):
        if self.is_training:
            ...
            self.gt_list = glob.glob(os.path.join(path, '*.txt'))
    ...
    def parse_data(self,gt_path):
        def get_words(gt):
            ...
            for line in reader:
                ...
                words.append(label)
            return words
        # self.str_queries = [v.strip() for v in open(os.path.join(self.floder, 'query.txt')).readlines()]
        # self.str_queries = [v.strip() for v in open(os.path.join(self.floder, 'query_full.txt')).readlines()]
        # print(self.str_queries)
        self.img_lists = glob.glob(os.path.join(self.floder, 'test/*.jpg'))
        ...
        for idx,img_path in enumerate(self.img_lists):
            words = get_words(img_path.replace('.jpg', '.txt'))
            for word in words:
...
class LSVTDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, use_difficult=False, transforms=None, is_train=True,augment=None):
        ...
        self.dataset = LSVT(data_dir, is_train)
    ...
    def __getitem__(self, idx):
        if self.is_train:
            ...
            polys = [self.expand_point(ps, NUM_POINT) for ps in polys]
    ...
    def expand_point(self, poly, num_point):
...
if __name__ == "__main__":
    data_dir = "/home/ymk-wh/workspace/datasets/retrieval_chinese/LSVT"
    dataset = LSVT(data_dir)
    text_dict = {}
