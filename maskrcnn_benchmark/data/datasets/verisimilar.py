...
class VeriSimilar(object):
    ...
    def getitem(self,index):
        ...
        img_path = os.path.join(self.image_floder, gt_name.replace('.txt','.jpg'))
        polys, texts = self.parse_gt_file(gt_path)
        # print(texts)
        # img = cv2.imread(img_path)
        # print(self.image_path_list[index])
        return img_path, polys, texts
NUM_POINT=7
class VeriSimilarDateset(torch.utils.data.Dataset):
    def __init__(self, data_dir, use_difficult=False, transforms=None, is_train=True,augment=None):
        ...
        self.dataset = VeriSimilar(data_dir, is_train)
