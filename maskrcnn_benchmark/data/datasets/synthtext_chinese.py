...
import cv2
def visual_partial_text(img, polys, texts):
    # img = cv2.imread(path)
    for poly, text in zip(polys, texts):
        print(poly.shape)
        minx,miny,maxx,maxy = int(np.min(poly[:,0])), int(np.min(poly[:,1])), int(np.max(poly[:,0])), int(np.max(poly[:,1]))
        img_patch = img[miny:maxy,minx:maxx]
        cv2.imwrite(os.path.join("temp", "{}.jpg".format(text)), img_patch)
