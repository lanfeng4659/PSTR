...
class ChineseCollect(object):
    ...
    def parse_data(self,gt_path):
        ...
        # print(queries)
        img_num = sum([len(os.listdir(os.path.join(gt_path,query))) for query in queries])
        ...
        self.str_queries_partial = [v.strip() for v in open(os.path.join(gt_path, 'v1_query_partial.txt')).readlines()]
