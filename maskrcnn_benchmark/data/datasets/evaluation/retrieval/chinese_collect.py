...
def chinese_collect_retrieval_evaluation(
    dataset,
    predictions,
    box_only,
    output_folder,
...
def re_ranking(retrieval_texts_embedding,predictions,y_trues):
    ...
    return y_scores
def norm(x):
    return nn.functional.normalize(x.tanh().view(x.size(0),-1))
def evaluate_box_proposals(
    ...
    # for image_id, prediction in enumerate(predictions):
    for i in range(len(dataset.dataset.img_lists)):
        # print(prediction.fields())
        prediction = torch.load(os.path.join(output_folder, 'img_{}.pth'.format(i)))
        if "words_embedding" in prediction.fields():
            words_embedding = prediction.get_field("words_embedding")
            words_embedding_nor = norm(words_embedding)
        # boxes = prediction.bbox.data.cpu().numpy()
        # scale = prediction.get_field("scale")
        # boxes[:,::2] *= scale[0]
        # boxes[:,1::2] *= scale[1]
        img_embedding = prediction.get_field("imgs_embedding")
        
        if img_embedding.size(0)==0:
            y_scores.append(torch.zeros([words_embedding_nor.size(0)]).to(img_embedding.device))
            continue
        img_embedding = norm(img_embedding)
