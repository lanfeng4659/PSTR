...
class Icdar17Dateset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if self.is_train:
            ...
            boxlist.add_field('polys',torch.as_tensor([self.expand_point(ps) for ps in order_polys.reshape([-1,4,2])], dtype=torch.float32))
            partial_polys = []
            partial_texts = []
            bag_ids = [len(polys)]
            assert len(polys)==len(tags), print(len(polys),len(tags))
            ...
            # print('data:', full_num,all_num)
            assert full_num == len(tags), print(full_num , len(tags))
            assert all_num-full_num == len(partial_texts), print(all_num-full_num , len(partial_texts))
