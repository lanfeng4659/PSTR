...
def main():
  ...
  if not run_on_remote:
    print(args)
    cmd_pretrain = "CUDA_VISIBLE_DEVICES=1,2 python -m torch.distributed.launch --nproc_per_node={0} --master_port 20000 tools/train_net.py \
           --config-file {1} \
  ...
  os.system("ln -s {} datasets".format(args.datasets_path))
  if run_on_remote:
  ...
  ours_test = "CUDA_VISIBLE_DEVICES=2 python {0} --config-file {1} \
   --OUTPUT_DIR {2}/finetune \
